...
- **Trivia**: `!vote 1-8` to pick a topic; `!a`/`!b`/`!c`/`!d` for answers (host reveals to advance).
...
- Winning topic is selected from chat votes (ties random among leaders; no votes defaults to topic #1), questions are generated (with fallbacks), and host presses **Reveal** to score and auto-advance through all 10.
...
- LocalStorage command shape:
  ...
  localStorage.setItem('streamCommand', JSON.stringify({
    ...
    command: 'add' | 'clear' | 'topics' | 'start' | 'vote' | 'answer' | 'reveal',
