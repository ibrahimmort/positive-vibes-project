...
if (!mongoUri || !sessionSecret) {
  console.error("FATAL ERROR: MONGODB_URI and SESSION_SECRET must be set in .env file");
  process.exit(1);
}
...
if (process.env.EMAIL_HOST && process.env.EMAIL_USER && process.env.EMAIL_PASS && process.env.EMAIL_FROM && process.env.EMAIL_TO) {
  ...
  transporter.verify()
    ...
    .catch(err => console.error('Nodemailer transporter configuration error:', err));
} else {
  console.warn('Email configuration missing. Contact form disabled.');
  transporter = null;
}
...
async function connectDB() {
  ...
  await db.collection('vibes').createIndex({ userId: 1, createdAt: -1 });
  await db.collection('vibes').createIndex({ vibeLocation: 1, createdAt: -1 });
  await db.collection('sessions').createIndex({ expires: 1 }, { expireAfterSeconds: 0 });
  await db.collection('weeklyThemes').createIndex({ startDate: -1 });
}
...
connectDB().then(() => {
  ...
  function isAuthenticated(req, res, next) {
      if (req.session && req.session.userId) { // Check session exists first
          next();
      } else {
  ...
  // --- Status Route (Handles Streak Reset & Badge Awarding) ---
  app.get('/api/auth/status', async (req, res) => {
    // Check if session and userId exist at the beginning
    if (!req.session || !req.session.userId) {
        console.log("[Status] No active session found.");
        return res.json({ loggedIn: false });
    }

    // If session exists, proceed within the try block
    const userIdFromSession = req.session.userId;
    console.log(`[Status] Checking status for session user ID: ${userIdFromSession}`);
    try {
        let userObjectId;
        try {
            userObjectId = new ObjectId(userIdFromSession);
        } catch (idError) {
             console.error(`[Status] Invalid user ID format in session: ${userIdFromSession}`, idError);
        ...
        const user = await db.collection('users').findOne(
            { _id: userObjectId },
            { projection: { email: 1, currentStreak: 1, longestStreak: 1, lastVibeWeekStart: 1, badges: 1 } }
        );

        if (!user) {
             console.warn(`[Status] User data not found in DB for session user ID: ${userIdFromSession}. Destroying session.`);
        ...
        const lastVibeWeekStartDate = user.lastVibeWeekStart;
        const now = new Date();
        ...
        // Check for missed week
        if (lastVibeWeekStartDate) {
            if (lastVibeWeekStartDate instanceof Date && lastVibeWeekStartDate.getTime() < previousWeekStart.getTime()) {
                 if (currentStreak !== 0) {
                     console.log(`[Streak Check] User ${user.email} missed a week. Resetting streak.`);
                     currentStreak = 0; needsDbUpdate = true; updateOperation.$set = { ...(updateOperation.$set || {}), currentStreak: 0 };
                 }
            }
        } else if (currentStreak !== 0) {
             console.log(`[Streak Check] User ${user.email} has no lastVibeWeekStart but streak > 0. Resetting streak.`);
             currentStreak = 0; needsDbUpdate = true; updateOperation.$set = { ...(updateOperation.$set || {}), currentStreak: 0 };
        }

        // Award Badges based on LONGEST streak
        const newlyEarnedBadges = [];
        for (const tier of BADGE_TIERS) { if (longestStreak >= tier.weeks && !userBadges.includes(tier.name)) { newlyEarnedBadges.push(tier.name); } }
        ...
        if (needsDbUpdate) {
            try {
                console.log(`[Status Update] Updating DB for ${user.email}:`, JSON.stringify(updateOperation));
                const updateResult = await db.collection('users').updateOne({ _id: user._id }, updateOperation);
                console.log(`[Status Update] DB update result for ${user.email}: Matched ${updateResult.matchedCount}, Modified ${updateResult.modifiedCount}`);
                if (newlyEarnedBadges.length > 0 && updateResult.modifiedCount > 0) { userBadges = [...userBadges, ...newlyEarnedBadges]; }
            } catch (updateError) {
                 console.error(`[Status Update] FAILED to update DB for user ${user.email}:`, updateError);
                 if (updateOperation.$set && updateOperation.$set.currentStreak === 0) { currentStreak = user.currentStreak || 0; } // Revert local if failed
            }
        }
         ...
         // Get last vibe timestamp for button state
        ...
        res.json({
            loggedIn: true,
            email: user.email,
            lastVibeTimestamp: lastVibe ? lastVibe.createdAt.toISOString() : null,
    ...
    // Catch errors from the main try block
        ...
        res.status(500).json({ loggedIn: false, message: "Error fetching status" });
    }
    // The initial if (!req.session...) handles the case where there's no session.
  }); // End of app.get('/api/auth/status', ...)

  // ... (forgot password, reset password routes remain the same) ...
  ...
  app.post('/api/vibes', isAuthenticated, async (req, res) => {
    ...
    try {
        ...
        catch (idError) { console.error(`[Vibes Route] Invalid user ID format in session: ${userId}`, idError); return res.status(400).json({ message: "Invalid user session." }); }
        ...
        if (existingVibeThisWeek) {
            nextWeekStartTime = getNextWeekStart(existingVibeThisWeek.createdAt);
            console.log(`[Vibes Route] User ${userEmailForLog} already pushed this week. Sending 429.`);
            return res.status(429).json({ message: "You've already spread your vibe this week!", nextAvailableTimestamp: nextWeekStartTime.toISOString() });
        }
        ...
        console.error(`[Vibes Route] UNEXPECTED ERROR for user ${userId}:`, error);
        res.status(500).json({ message: "Server error recording vibe" });
    }
  });
  ...
  app.listen(PORT, () => { console.log(`Server running at http://localhost:${PORT}/`); });
     ...
     console.error("Could not connect to MongoDB or other startup error", err);
     process.exit(1);
 });
