...
async function connectDB() {
  ...
  try {
      ...
      db = client.db('positiveVibesDB'); // Use your actual DB name
...
connectDB().then(() => {
  ...
  app.use(session({
    ...
    store: MongoStore.create({
        mongoUrl: mongoUri,
        dbName: 'positiveVibesDB',
        collectionName: 'sessions',
        ttl: 14 * 24 * 60 * 60 // 14 days
    }),
    cookie: {
        secure: process.env.NODE_ENV === 'production', // CRITICAL for HTTPS on Fly.io
        httpOnly: true, // Prevent client-side JS access
        sameSite: 'lax', // Recommended default for most cases
  ...
  function isAuthenticated(req, res, next) {
      if (req.session && req.session.userId) { // Check session and userId exist
  ...
  app.post('/api/auth/login', async (req, res) => {
    ...
    try {
        ...
        if (!user || !user.passwordHash) { console.log(`[Login Failed] Invalid credentials for email: ${email}`); return res.status(401).json({ message: "Invalid email or password" }); }
        const match = await bcrypt.compare(password, user.passwordHash);
        if (!match) { console.log(`[Login Failed] Password mismatch for email: ${email}`); return res.status(401).json({ message: "Invalid email or password" }); }
        ...
        req.session.regenerate((err) => {
            ...
            console.log("[Login Success] Full session object after regenerate:", JSON.stringify(req.session));
  ...
  app.post('/api/auth/logout', (req, res) => {
    const sessionId = req.session ? req.session.id : 'N/A';
    if (req.session) {
    ...
    } else { console.log("[Logout] No session found to destroy."); res.json({ message: "Logged out (no session)" }); }
  });
  ...
  app.get('/api/auth/status', async (req, res) => {
    ...
    console.log(`[Status] Checking status for session user ID: ${userIdFromSession}, Session ID: ${req.session.id}`);
    try {
  ...
  app.post('/api/auth/forgot-password', async (req, res) => {
      ...
      if (!transporter) { console.error("[Forgot PW] Request received, but email transporter is not configured."); return res.json({ message: "If an account with that email exists, a password reset link has been sent." }); }
      try {
          const user = await db.collection('users').findOne({ email });
          if (!user) { console.log(`[Forgot PW] Password reset requested for non-existent email: ${email}`); return res.json({ message: "If an account with that email exists, a password reset link has been sent." }); }
          ...
          const resetLink = `${frontendUrl}/reset-password.html?token=${token}`; // Use frontendUrl
          console.log(`[Forgot PW] Generated reset link: ${resetLink}`);
          ...
          try { await transporter.sendMail(mailOptions); console.log(`[Forgot PW] Password reset email sent to ${user.email}`); res.json({ message: "If an account with that email exists, a password reset link has been sent." }); }
          catch (emailError) { console.error("[Forgot PW] Error sending password reset email:", emailError); res.json({ message: "If an account with that email exists, a password reset link has been sent." }); }
      } catch (error) { console.error("[Forgot PW] Error in forgot password process:", error); res.json({ message: "If an account with that email exists, a password reset link has been sent." }); }
  });
  ...
  // --- Stats Route ---
  app.get('/api/stats', async (req, res) => { try { const thisWeekStart = getStartOfWeek(new Date()); const stats = await db.collection('vibes').aggregate([ { $facet: { "weekly": [ { $match: { createdAt: { $gte: thisWeekStart } } }, { $count: "count" } ], "total": [ { $count: "count" } ] } } ]).toArray(); const weeklyVibes = stats[0]?.weekly[0]?.count || 0; const totalVibes = stats[0]?.total[0]?.count || 0; res.json({ weeklyVibes, totalVibes }); } catch (error) { console.error("Error fetching stats:", error); res.status(500).json({ message: "Server error fetching stats" }); } });
  // --- Theme Route ---
  app.get('/api/theme/current', async (req, res) => { try { const now = new Date(); const startOfCurrentWeek = getStartOfWeek(now); const currentTheme = await db.collection('weeklyThemes').findOne( { startDate: { $lte: startOfCurrentWeek } }, { sort: { startDate: -1 } } ); if (currentTheme) { res.json({ theme: currentTheme.theme, suggestions: currentTheme.suggestions }); } else { res.status(404).json({ theme: "Stay Tuned!", suggestions: ["A new theme is coming soon.", "Keep spreading positive vibes!", "Check back next week."] }); } } catch (error) { console.error("Error fetching current theme:", error); res.status(500).json({ message: "Server error fetching theme" }); } });
  // --- Map Data Route ---
  app.get('/api/map-data', async (req, res) => { try { const now = new Date(); const sevenDaysAgo = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000); const mapData = await db.collection('vibes').aggregate([ { $match: { createdAt: { $gte: sevenDaysAgo }, vibeLocation: { $ne: null, $ne: "" } }}, { $group: { _id: "$vibeLocation", count: { $sum: 1 } }}, { $project: { _id: 0, location: "$_id", count: "$count" }}, { $sort: { count: -1 } }, { $limit: 200 } ]).toArray(); res.json(mapData); } catch (error) { console.error("Error fetching map data:", error); res.status(500).json({ message: "Server error fetching map data" }); } });
  // --- Contact Form Route ---
  app.post('/api/contact', async (req, res) => { const { message } = req.body; if (!message || message.trim().length === 0) return res.status(400).json({ message: "Message content cannot be empty." }); if (!transporter) { console.error("[Contact] Contact form submitted, but email transporter is not configured."); return res.status(500).json({ message: "Message received, but server email configuration is incomplete." }); } let senderInfo = "An anonymous user"; let replyToEmail = undefined; if (req.session && req.session.userId && req.session.email) { senderInfo = `User ${req.session.email}`; replyToEmail = req.session.email; } const mailOptions = { from: process.env.EMAIL_FROM, to: process.env.EMAIL_TO, replyTo: replyToEmail, subject: `PVP Contact Form Message from ${senderInfo}`, text: `Sender: ${senderInfo}\n\nMessage:\n${message}`, html: `<p><b>Sender:</b> ${senderInfo}</p><p><b>Message:</b></p><pre>${message.replace(/</g, "&lt;").replace(/>/g, "&gt;")}</pre>`, }; try { let info = await transporter.sendMail(mailOptions); console.log('[Contact] Contact email sent successfully. Message ID: %s', info.messageId); res.json({ message: "Message sent successfully! Thank you." }); } catch (error) { console.error("[Contact] Error sending contact email via nodemailer:", error); res.status(500).json({ message: "Failed to send message due to a server error. Please try again later." }); } });
